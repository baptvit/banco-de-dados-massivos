...
def go_search(
    ...
    go_search_params = [
        go_benchmark,  # path to the go executable
        "locust",
        "-u",
        uri,  # host:port
        ...
        "-q",
        search_vector_file,  # vector file path for searching
        "-s",
        json.dumps(query_json, indent=2),
        "-p",
        str(concurrent_num),  # concurrent number
        "-f",
        output_format,  # format of output
        "-t",
        str(during_time),  # total time of concurrent, second
        "-i",
        str(interval),  # log print interval, second
        "-l",
        str(log_path),  # log file path
    ]
...
if __name__ == "__main__":
    ...
    go_benchmark = sys.argv[
        2
    ]  # path to benchmark binary file  e.g. "/home/benchmark-mac"
    ...
    search_vector_file = (
        "search_vector_file.json"  # vectors in json file that would be searched
    )
    ...
    go_search(
        ...
        search_vector_file=search_vector_file,
